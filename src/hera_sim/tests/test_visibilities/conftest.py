...
def zenith_sky_model(uvdata2):
    return make_point_sky(
        uvdata2,
        ra=np.array([0.0]),
        dec=np.array([uvdata2.telescope.location.lat.rad]),
        align=True,
    )
...
def horizon_sky_model(uvdata2):
    return make_point_sky(
        uvdata2,
        ra=np.array([0.0]),
        dec=np.array([uvdata2.telescope.location.lat.rad + np.pi / 2]),
        align=True,
    )
...
def twin_sky_model(uvdata2):
    return make_point_sky(
        ...
        dec=np.array(
            [
                uvdata2.telescope.location.lat.rad + np.pi / 4,
                uvdata2.telescope.location.lat.rad,
            ]
        ),
        align=True,
    )
...
def sky_model(uvdata):
    return make_point_sky(
        uvdata,
        ra=np.array([0.0]) * rad,
        dec=np.array(uvdata.telescope.location.lat.rad) * rad,
        align=False,
    )
