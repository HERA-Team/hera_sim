...
def test_match_antennas_default_settings(base_config, base_sim):
    ...
    assert np.all(modified_sim.data.telescope.antenna_numbers == list(overlap_array.keys()))
...
def test_match_antennas_all_kwargs_false(base_sim, base_config):
    ...
    assert arrays_are_equal(overlap_array, modified_sim.antpos)
    assert np.all(
        modified_sim.data.telescope.antenna_numbers == list(target_to_ref_ant_map.keys())
    )
...
def test_match_antennas_using_ENU_positions(base_sim, base_config):
    ...
    assert np.all(modified_sim.data.telescope.antenna_numbers == list(overlap_array_ENU.keys()))
