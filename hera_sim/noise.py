...
def sky_noise_jy(Tsky, fqs, lsts, bm_poly=HERA_BEAM_POLY, inttime=10.7):
    ...
    T2jy.shape = (1, -1)
    Vnoise_jy = (
        T2jy * Tsky / np.sqrt(inttime * B)
    )  # see noise_study.py for discussion of why no facItor of 2 here
    return (
        white_noise(Vnoise_jy.shape) * Vnoise_jy
    )  # generate white noise with amplitude set by Vnoise_jy
