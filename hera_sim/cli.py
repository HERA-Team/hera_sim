...
@click.option(
    ...
    help="path to output file. Over-rides config if passed.",
    default=None,
)
@click.option("-v", "--verbose", count=True)
@click.option(
    "-sa", "--save-all", count=True, help="Choose whether to save all data products."
)
@click.option(
    "-c",
    "--clobber",
...
def run(input, outfile, verbose, save_all, clobber):
    ...
    if bda_params and bda is None:
        raise ImportError(
            "You have defined BDA parameters but do not have "
    ...
    if defaults and config_params:
        warnings.warn(
            "You have chosen to use a default configuration in "
    ...
    if not (config_params or defaults):
        warnings.warn(
            "You have specified neither defaults nor configuration "
            ...
            if component in content.keys():
                for model, params in content[component].items():
                    ...
                    if save_all:
                        ...
                        sim_copy.data.history = sim.data.history.replace(
    ...
    sim.data.history += "\nSimulation from configuration file: {cfg}".format(cfg=input)
    sim.write(
        outfile, save_format=filing_params["output_format"], **filing_params["kwargs"]
    )
