...
class TestVisCpu(unittest.TestCase):
    def test_shapes(self):
        ...
        self.assertRaises(
            AssertionError, vis.vis_cpu, antpos.T, 0.15, eq2tops, crd_eq, I_sky, bm_cube
        )
        self.assertRaises(
            AssertionError, vis.vis_cpu, antpos, 0.15, eq2tops.T, crd_eq, I_sky, bm_cube
        )
        self.assertRaises(
            AssertionError, vis.vis_cpu, antpos, 0.15, eq2tops, crd_eq.T, I_sky, bm_cube
        )
        self.assertRaises(
            AssertionError, vis.vis_cpu, antpos, 0.15, eq2tops, crd_eq, I_sky, bm_cube.T
        )
    ...
    def test_values(self):
        ...
        v = vis.vis_cpu(antpos, 1.0, eq2tops, crd_eq, I_sky * 0, bm_cube)
        np.testing.assert_equal(v, 0)
        v = vis.vis_cpu(antpos, 1.0, eq2tops, crd_eq, I_sky, bm_cube * 0)
        np.testing.assert_equal(v, 0)
        # For co-located ants & sources on sky, answer should be sum of pixels
        v = vis.vis_cpu(antpos, 1.0, eq2tops, crd_eq, I_sky, bm_cube)
