...
def test_bad_npz():
    ...
    np.savez(
        os.path.join(temp_dir, "bad_shape"),
        lsts=lsts,
        freqs=freqs,
        tsky=bad_tsky,
        meta=meta,
...
def test_bad_params():
    ...
    # bad interpolator
    # TODO: once one raises an error, won't the test just pass and the rest won't be run?
    Beam(os.path.join(temp_dir, "some_npy.npy"), interpolator="something")
    # bad datafile extension v1
    Beam(os.path.join(temp_dir, "some_npy.npy"), interpolator="interp1d")
    # bad datafile extension v2
    Beam(os.path.join(temp_dir, "no_freqs.npz"), interpolator="poly1d")
    # bad keys
    Beam(os.path.join(temp_dir, "no_freqs.npz"), interpolator="interp1d")
    Beam(os.path.join(temp_dir, "no_values.npz"), interpolator="interp1d")
    # nonexistent file
    Beam(os.path.join(temp_dir, "not_a_file.npz"), interpolator="interp1d")
