...
class TestReflections(unittest.TestCase):
    ...
    def test_auto_reflection(self):
        ...
        outvis = reflections.auto_reflection(
            self.Tsky, self.freqs, 1e-1, 300, 1, conj=True
        )
        ovfft = np.fft.fft(
            outvis * windows.blackmanharris(len(self.freqs))[None, :], axis=1
        )
    ...
    def test_cross_reflection(self):
        ...
        outvis = reflections.cross_reflection(
            self.vis, self.freqs, self.Tsky, 1e-2, 300, 0
        )
        ovfft = np.fft.fft(
            outvis * windows.blackmanharris(len(self.freqs))[None, :], axis=1
        )
        ...
        outvis = reflections.cross_reflection(
            self.vis, self.freqs, self.Tsky, 1e-2, 300, 1, conj=True
        )
        ovfft = np.fft.fft(
            outvis * windows.blackmanharris(len(self.freqs))[None, :], axis=1
        )
