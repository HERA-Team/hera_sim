...
#####################################
SIMULATORS = (vis.VisCPU, vis.VisGPU, vis.HealVis)
#####################################
...
def test_visgpu_res_autocorr(uvdata):
...
def test_simulator_comparison(uvdata):
    ...
    assert viscpu.shape == visgpu.shape
    assert np.testing.assert_allclose(viscpu, visgpu)
