...
class TestForegrounds(unittest.TestCase):
    def test_rough_delay_filter(self):
        ...
        np.testing.assert_array_less(
            np.mean(np.abs(_n1_filt), axis=0),
    ...
    def test_rough_fringe_filter(self):
        ...
        fr_max1 = utils.calc_max_fringe_rate(0.1, bl_len_ns)
        fr_max2 = utils.calc_max_fringe_rate(0.2, bl_len_ns)
        ...
        np.testing.assert_allclose(
            np.sqrt(np.mean(np.abs(_n1_filt[inside_filter, :]) ** 2, axis=-1)),
            np.sqrt(lsts.size),
            rtol=0.15,
        )
