...
def rfi_stations(fqs, lsts, stations=None, season=None, rfi=None):
    """
    ...
    if stations is None:
        ...
        try:
            rfi_file = glob.glob('{}/*RFI*.npy'.format(DATA_PATH))[0]
            stations = np.load(rfi_file)
        except:
            warnings.warn('RFI Station parameter file not found.')
