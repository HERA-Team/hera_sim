...
class PRISim(VisibilitySimulator):
    ...
    def interferometer(self):
        ...
        baseline_vectors = np.array(
            [antpos[ant[0]] - antpos[ant[1]] for ant in antpairs]
        )

        return intf.InterferometerArray(
            labels=[
                ("A" + str(i), str(baseline_vectors[i])) for i in range(len(antpairs))
            ],
    ...
    def validate(self):
        ...
        assert np.all(
            self.uvdata.integration_time == self.uvdata.integration_time[0]
        ), "PRISim assumes a single integration time for all times"
    ...
    def sky_model(self):
        ...
        return catalog.SkyModel(
            init_parms=dict(
                ...
                location=np.array(
                    [self._sky_coords.ra.deg, self._sky_coords.dec.deg]
                ).T,
                spec_type="spectrum",
    ...
    def _astropy_times(self):
        return [
            Time(t, format="jd", scale="utc", location=self._earth_location)
            for t in self.times
        ]
    ...
    def _prisim_telescope(self):
        if isinstance(self.beams[0], pyuvdata.UVBeam):
            ...
            if self.beams[0].type == "uniform":
                ...
                raise ValueError(
                    "beam type if analytic must be uniform, airy or gaussian"
                )
    ...
    def run_region_of_interest(self):
        ...
        for i, (ind, time) in enumerate(zip(indices, self.times)):
            if len(ind) == 0:
                ...
                if not np.all(src_altaz[:, 0] >= 0):
                    warnings.warn(
                        "some sources are below horizon! Removing those sources manually. Minimum alt={}".format(
                            src_altaz[:, 0].min()
                        )
                    )
                ...
                pbinfo = {
                    "pointing_coords": "altaz",
                    "pointing_centre": np.array([90.0, 270.0]),
                }
                ...
                if isinstance(self.beams[0], pyuvdata.UVBeam):
                    beam = (
                        self.beams[0]
                        .data_array[0, 0, 0 if self.beam_pol == "x" else 1, :, :]
                        .T
                    )
        ...
        roi.save(self.aux_file_loc, tabtype="BinTableHDU", overwrite=True, verbose=True)
    ...
    def _simulate(self):
        ...
        for j, (ind, time) in enumerate(zip(indices, self.times)):
            ...
            bandpass = getattr(
                self.beams[0], "bandpass_array", np.ones_like(self.uvdata.freq_array[0])
            )
            ...
            self.interferometer.observe(
                ...
                pb_info={
                    "pointing_centre": np.array([90.0, 270]),  # hard-coded to zenith
                    "pointing_coords": "altaz",
                },
                brightness_units="Jy",
